...
""" userge module containing commands for keeping global notes. """

from userge.events import register


@userge.on_cmd(outgoing=True, pattern=r"\$\w*", disable_errors=True)
async def on_snip(event):
    """ Snips logic. """
    try:
        from userge.modules.sql_helper.snips_sql import get_snip
...
@userge.on_cmd(outgoing=True, pattern="^\.snip (\w*)")
async def on_snip_save(event):
    """ For .snip command, saves snips for future use. """
    try:
        from userge.modules.sql_helper.snips_sql import add_snip
...
@userge.on_cmd(outgoing=True, pattern="^\.snips$")
async def on_snip_list(event):
    """ For .snips command, lists snips saved by you. """
    try:
        from userge.modules.sql_helper.snips_sql import get_snips
    except AttributeError:
        await event.edit("`Running on Non-SQL mode!`")
        return
...
@userge.on_cmd(outgoing=True, pattern="^\.remsnip (\w*)")
async def on_snip_delete(event):
    """ For .remsnip command, deletes a snip. """
    try:
        from userge.modules.sql_helper.snips_sql import remove_snip
    except AttributeError:
        await event.edit("`Running on Non-SQL mode!`")
        return
